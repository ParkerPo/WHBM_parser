...
function parser(url ,cb){
	request.get(url,function (err,res,body){
		...
		$('a.product-name').each(function(i,item){
			names[seed+i]=$(this).clone().children().remove().end().text().trim();
			//console.log(names[i]);
			rating[seed+i]=$(this).children('.BVRRRatingNormalOutOf').children('.BVRRRatingNumber').text();
            rating_count[seed+i]=$(this).children('.BVRRRatingNormalOutOf').children('.BVRRRatingReviewsCount').text();
			var price=$(this).parent().children('div.product-price')
			price_now[seed+i]=price.children('span.product-price-now').text().replace(/\D+/,"").trim();
			price_was[seed+i]=price.children('span.product-price-was').text().replace(/\D+/,"").replace(/\)/,"").trim();
			product_price[seed+i]=price.children('span.product-price').text().replace(/\D+/,"").trim();
            product_id[seed+i]=$(this).attr('onclick').replace("s_objectID='product_","").replace("_name';","");
		});
		...
		for (var i=seed;i<names.length;i++){
        ...
        seed = product_id.length;
        cb()
	});
}

function update_summary( callback ) {
    var sys_time = new Date;
    var time = sys_time.getFullYear()+"-"+(sys_time.getMonth()+1)+"-"+sys_time.getDate()+"-"+sys_time.getHours();
    //把資訊加到舊的file裡面
    if (series.length==0){
...
function clear_detail( callback ){
    rating=[];
    rating_count=[];
    names=[];
    price_now=[];
...
async.whilst(
    function (){
        ...
        async.series([
            ...
            function ( callback ){
                update_summary( callback )
            },
            function ( callback ){
                clear_detail( callback )
            }
        ],function(err){
            i++;
            //sleep.sleep(30)
